...
struct grid
{
    // Trying my hardest to avoid backing this with a 2D array
...
static intersections GlobalIntersections;
static int GlobalPolylineCount;
...
static void
Initialize(grid *Grid, int Width, int Height)
{
    Grid->Width = Width;
    Grid->Height = Height;
...
static void
Add(intersections *Intersections, v2 P)
{
    Intersections->Store[Intersections->Count++] = P;
}

static void
DrawPolyline(grid *Grid, const char *LineSpec)
{
    char Stamp = ('a' + GlobalPolylineCount++);
...
//        printf("Move: %d, %d\n", Move.Type, Move.Distance);
        if(IsValid(Move))
        {
            if(StepP != Grid->CentralPortP)
            {
                Place(Grid, StepP, Stamp);
            }
            else
            {
                Place(Grid, StepP, 'o');
            }
            if(Move.Type == MoveType_Up)
            {
                ...
                for(int Y = (StepP.y + 1);
                    ...
                    Y++)
                {
                    v2 CurrentP = V2(StepP.x, Y);
                    Assert((0 <= CurrentP.x) && (CurrentP.x < Grid->Width));
                    Assert((0 <= CurrentP.y) && (CurrentP.y < Grid->Height));
#if 1
                    if((Peek(Grid, CurrentP) != '.') &&
                       (Peek(Grid, CurrentP) != Stamp))
                    {
                        Place(Grid, CurrentP, 'X');
                        Add(&GlobalIntersections, CurrentP);
                    }
                    else
                    {
                        Place(Grid, CurrentP, Stamp);
                    }
#endif
                }
                StepP.y = EndY;
            }
            else if(Move.Type == MoveType_Down)
            {
                int EndY = (StepP.y - Move.Distance);
                for(int Y = (StepP.y - 1);
                    Y >= EndY;
            ...
            else if(Move.Type == MoveType_Left)
            {
            ...
            else if(Move.Type == MoveType_Right)
            {
                int EndX = (StepP.x + Move.Distance);
                for(int X = (StepP.x + 1);
                    X <= EndX;
...
Absi(int I)
{
    int Result = (int)Abs((f32)I);

    return(Result);
}

static int
ComputeDist(v2 A, v2 B)
{
